...
class Conn():
    ...
    def __init__(self,db):
        self.db = db
        self.server_info = conn_info.server_infos[db]
        #self.ssh = Ssh(db)     # ssh 터널링을 사용할 경우
        self.ssh = SshDummy(db)     # ssh 터널링을 사용하지 않을 경우 
        self.conn = None
        self.curType = 'dict'
    ...
    def sshDbConn(self):
        ...
        """
        server_info = self.server_info
        if hasattr(self,'ssh') and type(self.ssh) is SshDummy :
            source_db_port = server_info["source_db_port"] if ("source_db_port" in server_info ) else 3306
            #print('source_db_port=',source_db_port)
            #print('server_info=',server_info)
            self.conn = pymysql.connect(host=server_info["source_endpoint"], port=source_db_port, user=server_info["source_db_user"], password=server_info['source_db_pwd'], charset='UTF8')  
        else:
